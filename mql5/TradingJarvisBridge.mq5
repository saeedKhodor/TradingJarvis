...
// 2. Indicator Handle Declarations & Structures (Line 40)
// 3. OnInit() - Initializes MT5 native iMA indicator handles (Line 65)
// 4. OnDeinit() - Releases indicator handles and cleans up resources (Line 140)
// 5. OnTick() - Triggered on live incoming ticks (Line 170)
// 6. OnTimer() - Periodic fallback export timer (Line 180)
// 7. UpdateAndExportIndicators() - Reads CopyBuffer and writes JSON to Common Files (Line 192)
// ===================

#property copyright "TradingJarvis AI"
#property link      "https://github.com/saeedKhodor/TradingJarvis"
#property version   "1.10"
#property description "J.A.R.V.I.S. Native MT5 Multi-Timeframe 9/21/50-EMA Indicator Bridge"

// --- Input Parameters
input group "=== Indicator Settings ==="
input int                InpEmaPeriod       = 9;              // Primary 9-EMA Period
input int                InpCascadeEma21    = 21;             // H1 Cascade 21-EMA Period
input int                InpCascadeEma50    = 50;             // H1 Cascade 50-EMA Period
...
int OnInit()
{
   Print("[J.A.R.V.I.S. Bridge] Initializing native MT5 Multi-Timeframe EMA handles for ", _Symbol, "...");
   ...
   h_h1_ema21 = iMA(_Symbol, PERIOD_H1,  InpCascadeEma21, 0, InpMaMethod, InpAppliedPrice);
   h_h1_ema50 = iMA(_Symbol, PERIOD_H1,  InpCascadeEma50, 0, InpMaMethod, InpAppliedPrice);
   ...
   Print("[J.A.R.V.I.S. Bridge] All 9 Multi-Timeframe + H1 Cascade EMA handles initialized successfully.");
...
void OnDeinit(const int reason)
{
   ...
   if(h_h1_ema21 != INVALID_HANDLE) IndicatorRelease(h_h1_ema21);
   if(h_h1_ema50 != INVALID_HANDLE) IndicatorRelease(h_h1_ema50);
...
void UpdateAndExportIndicators()
{
   ...
   // Read exact buffer values (shift 0 = current live bar)
   ...
   double ema_h1_ema21 = GetEmaVal(h_h1_ema21, 0);
   double ema_h1_ema50 = GetEmaVal(h_h1_ema50, 0);
   ...
   json += "    \"H1_EMA21\": " + DoubleToString(ema_h1_ema21, _Digits) + ",\n";
   json += "    \"H1_EMA50\": " + DoubleToString(ema_h1_ema50, _Digits) + ",\n";
   json += "    \"H4\": "       + DoubleToString(ema_h4, _Digits) + ",\n";
   json += "    \"D1\": "       + DoubleToString(ema_d1, _Digits) + "\n";
   json += "  }\n";
   json += "}\n";

   // Write to Common Files directory
   ...
   if(InpVerboseLogs)
   {
      Print("[J.A.R.V.I.S. Bridge] Exported Cascade EMAs: H1_9=", DoubleToString(ema_h1, 2), " H1_21=", DoubleToString(ema_h1_ema21, 2), " H1_50=", DoubleToString(ema_h1_ema50, 2));
   }
}
